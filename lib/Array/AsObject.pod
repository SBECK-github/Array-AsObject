...
available in L<Template::Toolkit> templates using the companion module
L<Template::Plugin::ListOps>.

It performs many of the same functions as the L<Set::Array> module,
but has better support for handling duplicate elements in
lists, and does not require the L<Want> module.  For more
details, please refer to the L</"HISTORY AND RATIONALE"> section
below.
...
This creates a new C<Array::AsObject> object. If C<@list> is passed in, this
is set to the initial list of elements.

If C<@list> is passed in, the following two are equivalent:
...
There is no restriction on the type of elements in C<@list>. They can be
scalars, references, objects, or C<undef>.
...
in C<$obj>. References and C<undef> values are ignored.

Every scalar in the list is one of the keys in C<%hash>.  The value of
C<%hash> is the number of times that element appears in the list.

   ($count,$vals) = $obj->as_hash(1);

This returns two hash references that describe ALL values in the
list, including references, objects, and C<undef>.

Every unique value in the list is assigned a label (which has no
significance). The hash C<%$vals> has the labels as keys and the
actual values as values for the keys.

The hash C<%$count> has the labels as keys and the values are the
...
Note that the two elements C<$i> and C<$j> are treated as different since
...
This returns the element at position C<$n> in the list, or a list of the
elements at the positions given in the list C<@n>.
...
This counts the number of times val appears in the list. If C<$val>
is not given, it counts the number of C<undef> elements in the list.

=item B<exists>

   $flag = $obj->exists(@val);

This returns 1 if every value exists in the list. If C<@val> is not
passed in, it tests for C<undef> values.
...
In list context, these return the index of every occurrence of C<$val> in
the list. If C<$val> is not passed in, the indices of all C<undef> elements
are returned.
...
$val can be a scalar, C<undef>, or a reference, and all will work
...
This sets all elements in the list to be C<undef> (preserving the length).
...
This removes any C<undef> objects from the list.
...
If $undef is 1, values are replaced with C<undef>. Otherwise, they
...
This sets the list index to the given value, or C<undef> if no value
is passedin.
...
If $start is omitted (or is C<undef>), it defaults to 0. If $length
is omitted (or C<undef>), it defaults to the end of the list.
...
error is encountered, C<undef> is returned, but no error is stored.
...
lists with duplicate elements), it came close enough to the
functionality I initially needed, so I wrote a wrapper module
(Template::Plugin::ListOps) around it to do most of what I wanted.
...
I looked at the Want module, but correcting it was far enough out of
my comfort zone that the best solution seemed to be to rewrite the
module without depending on Set::Array. This would also allow me to
add the functionality that I wanted.
...
since some of the functionality I wanted was the enhanced duplicate
...
L<Set::Array> - a module with similar functionality
